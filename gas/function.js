...
function testDate() {
  ...
  const tomorrowEvents = myCalendar.fetchEventsForDay(tomorrow)
...
function tomorrowEventsPush (){
  ...
  for(const event of tomorrowEvents){
    ...
    let desctiption = event.getDescription();
    if (!!desctiption){
      message+=`【備考】\n${desctiption}\n`
    }
  ...
  if (tomorrowEvents.length > ng_title_count){
    ...
    let tomorrow_cells = ss.getRange(3,ss.getLastColumn(),ss.getLastRow()-2,1)
    tomorrow_cells.setValue([3])
